...
//   //처음에 header의 text가 나오게 하기 위해 scrollMove 실행
 scrollMove();
})
...
function scrollMove() {
  ...
  if(scrollNow >= headerHeight)
  {
    ...
    $('.inner-bar').each(function (){
      //.inner-bar의 다음 요소인 .percentage의 텍스트를 percentBar로 저장
      $(this).animate({width: $(this).next().text()}, 1500);
    })
  }
...
function textUp() {
  ...
  $('.vertical-center').each(function(index){
    if($(window).scrollTop() >= $(this).parents('section').position().top)
    {
...
//       }else {
//         var interScroll = setTimeout(frame, 5);
//         pv += 3;
